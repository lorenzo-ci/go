...
require_once 'schemas/zones-schema.php'; // Ranges schema
// Table fields, length and keys
$_OMP_tbl_fld = $_OMP_zones_fld;
$_OMP_fld_len = $_OMP_zones_len;
$_OMP_tbl_key = $_OMP_zones_key;
/**
* SQL code
*/
...
function OMP_makeVars()
{
    ...
    if ($_OMP_get['action'] == 'del') {
        ...
        $_OMP_table_data = array($_OMP_rec['pkey'], $_OMP_rec['note']);
        return;
    } elseif ($_OMP_get['action'] == 'read') {
        $_OMP_html['rec0'] = $_OMP_rec['pkey'];
        $_OMP_html['rec1'] = $_OMP_rec['note'];
        if ($_OMP_get['list']) {
            $_OMP_html['label0'] = '';
            $_OMP_html['label1'] = '';
            /* mdc-list */
            ...
            $_OMP_html['label0'] = $_OMP_LC[106];
            $_OMP_html['label1'] = $_OMP_LC[117];
        }
        $_OMP_html['supplier_link'] = OMP_popLink(
                $_OMP_TPL[9], OMP_PATH.'suppliers.php?'.
...
*/
case 'read': {
    $_OMP_tpl = OMP_TPL_READ.'61, 62, 2100';
    $_OMP_lcl = OMP_LCL_READ.'106, 117, 126, 127, 504';
