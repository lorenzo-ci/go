...
// +----------------------------------------------------------------------+
//
// $Id: enduses.php,v 0.8 $
//
// Manage enduses records
//
...
$_OMP_tbl_fld = $_OMP_enduses_fld;
$_OMP_fld_len = $_OMP_enduses_len;
$_OMP_tbl_key = $_OMP_enduses_key;
/**
* SQL code
*/
$_OMP_sql['select'] = 'SELECT '.$_OMP_tbl_fld['pkey'].' AS pkey, '.
    $_OMP_tbl_fld['name'].' AS name FROM '.$_OMP_tables[$_OMP_tbl];
...
{
    global $_OMP_html, $_OMP_LC, $_OMP_get, $_OMP_rec,
    ...
    if ($_OMP_get['action'] == 'del') {
        $_OMP_table_header = array($_OMP_LC[300], $_OMP_LC[301]);
        $_OMP_table_data = 
            array($_OMP_rec['pkey'], $_OMP_rec['name']);
...
case 'read': {
    ...
    $_OMP_lcl = OMP_LCL_READ.'3, 300, 301';
...
*/
case 'edit': {
    $_OMP_tpl = OMP_TPL_EDIT.'301';
    $_OMP_lcl = OMP_LCL_EDIT.'3, 300, 301';
...
*/
case 'filter': {
    $_OMP_tpl = OMP_TPL_FILTER.'300';
    $_OMP_lcl = OMP_LCL_FILTER.'3, 300, 301';
    $_OMP_page_title_lcl = 3;
    $_OMP_include_tpl = 300;
...
*/
case 'new': {
    $_OMP_tpl = OMP_TPL_NEW.'301';
    $_OMP_lcl = OMP_LCL_NEW.'3, 300, 301';
    // Check if insert-button was pushed
    if (isset($_POST['insert_button'])) {
...
case 'del': {
    ...
    $_OMP_lcl = OMP_LCL_DEL.'3, 300, 301';
    $_OMP_datatypes = array('text');
    if (isset($_POST['insert_button'])) {
